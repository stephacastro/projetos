...
def cadastrar_livro(livro: schemas.Cadastrar_livro, db: Session = Depends(get_db)):
    ...
    return crud.cadastrar_livro(db=db, livro=livro)
...
# Remover livro
@app.delete("/deletar-livro/", response_model=schemas.Livro)
def deletar_livro(livro: schemas.Deletar_livro, db: Session = Depends(get_db)):
    db_livro = crud.deletar_livro(db, livro=livro)
